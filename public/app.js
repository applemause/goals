...
const translations = {
  en: {
    ...
    takeStep: 'Progress', progressHistoryHint: 'Progress will be saved to history', addStagesFirst: 'Add milestones in the editor first.', history: 'History', historyAndNotes: 'History and notes', historyEmpty: 'Nothing here yet.', notePlaceholder: 'What is worth remembering about this goal?', addNote: 'Add note', addOne: 'Add one', completed: 'Completed', total: 'Total',
  ...
  de: {
    ...
    takeStep: 'Fortschritt', progressHistoryHint: 'Der Fortschritt wird im Verlauf gespeichert', addStagesFirst: 'Füge zuerst Etappen im Editor hinzu.', history: 'Verlauf', historyAndNotes: 'Verlauf und Notizen', historyEmpty: 'Noch ist hier nichts.', notePlaceholder: 'Was möchtest du zu diesem Ziel festhalten?', addNote: 'Notiz hinzufügen', addOne: 'Eins hinzufügen', completed: 'Erledigt', total: 'Gesamt',
  ...
  ru: {
    ...
    takeStep: 'Прогресс', progressHistoryHint: 'Прогресс сохранится в истории', addStagesFirst: 'Сначала добавьте этапы в редакторе.', history: 'История', historyAndNotes: 'История и заметки', historyEmpty: 'Пока здесь тихо.', notePlaceholder: 'Что важно запомнить об этой цели?', addNote: 'Добавить заметку', addOne: 'Добавить один', completed: 'Выполнено', total: 'Всего',
...
function renderGoal() {
  ...
  state.screen = 'goal';
  app.innerHTML = `<div class="app-shell"><section class="panel"><div class="view-nav"><button class="back" data-back>‹ ${t('goals')}</button><div class="view-actions">${languageSwitcher()}<button class="edit-structure" data-edit-structure>${editIconMarkup()}<span>${t('edit')}</span></button></div></div><header class="detail-head"><div class="detail-summary"><h1 class="goal-title">${escapeHtml(goal.title)}</h1><div class="goal-value">${valueMarkup(goal)}</div>${goal.meta ? `<div class="goal-meta">${escapeHtml(goal.meta)}</div>` : ''}</div><div class="detail-progress">${overallProgressMarkup(goal)}${progressMarkup(goal, true)}</div></header><section class="detail-section work-section"><div class="section-title-row"><h2>${t('takeStep')}</h2></div><ul class="work-list">${goal.milestones.length ? goal.milestones.map(workMilestoneMarkup).join('') : `<li class="goal-meta">${t('addStagesFirst')}</li>`}</ul></section><details class="history-details"><summary><span>${t('historyAndNotes')}</span><span class="history-count">${goal.events.length}</span></summary><div class="history-content"><ul class="event-list">${goal.events.length ? goal.events.map((event) => eventMarkup(event, goal)).join('') : `<li class="goal-meta">${t('historyEmpty')}</li>`}</ul><form class="form add-event note-form" data-add-event><label>${t('note')}<textarea name="title" maxlength="120" rows="3" placeholder="${t('notePlaceholder')}" required></textarea></label><button class="button" type="submit">${t('addNote')}</button></form></div></details></section></div>`;
  bindLanguageSwitcher();
  ...
  app.querySelectorAll('[data-increment-progress]').forEach((button) => button.addEventListener('click', () => {
    const milestone = goal.milestones.find((item) => item.id === button.dataset.incrementProgress);
    if (milestone) incrementProgress(milestone, button);
  }));
  app.querySelector('[data-add-event]').addEventListener('submit', addEvent);
}

async function incrementProgress(milestone, button) {
  button.disabled = true;
  try {
    await api(`/api/milestones/${milestone.id}/progress`, { method: 'POST', body: JSON.stringify({ workspaceId: state.workspaceId, amount: 1 }) });
    await refresh();
    renderGoal();
  } catch (error) {
    button.disabled = false;
